...
""" Add keyfile argument to parser  """
parser.add_argument("-k", "--keyfile", help='Caesar cipher key file', required=False)
...
try:
    ciphertext = open(args.decrypt, "rb").read().decode("ascii")
    try:
        plaintext = open(args.encrypt, "rb").read().decode("ascii")
...
if encrypting:
    ...
    # exit(1) not needed here because the program exits in the else block above

    """ ****************************** decryption code begins below here ******************************************* """
    # code deleted as to not share solution 
